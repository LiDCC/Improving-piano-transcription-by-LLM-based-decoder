...
class Vel_linear(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear = nn.Linear(128, 1)

    def forward(self, output_velocity_B):
        out_vel = self.linear(output_velocity_B).squeeze(-1)
        out_vel = torch.sigmoid(out_vel)
        return out_vel

def softargmax(x, beta=10):
    # x 的形状应为 [batch_size, n_frame, n_note, n_velocity]
    # 计算 softmax
    x_soft = F.softmax(x * beta, dim=-1)

    # 创建一个与 velocity 维度对应的 indices tensor
    indices = torch.arange(x.size(-1)).to(x.device).float()

    # 计算 softargmax
    softargmax = torch.sum(x_soft * indices, dim=-1) / x.size(-1)
    return softargmax
...
class Model_SPEC2MIDI(nn.Module):
    ...
    def forward(self, input_spec):
        ...
        output_onset_A, output_offset_A, output_mpe_A, output_velocity_A, attention, output_onset_B, output_offset_B, output_mpe_B, output_velocity_B, midi_time = self.decoder_spec2midi(enc_vector)
        output_velocity_A_soft = softargmax(output_velocity_A, beta=10)
        output_velocity_B_soft = softargmax(output_velocity_B, beta=10)
        ...
        return output_onset_A, output_offset_A, output_mpe_A, output_velocity_A, attention, output_onset_B, output_offset_B, output_mpe_B, output_velocity_B, midi_time, output_velocity_A_soft, output_velocity_B_soft
...
class Decoder_SPEC2MIDI(nn.Module):
    ...
    def forward(self, enc_spec):
        ...
        return output_onset_freq, output_offset_freq, output_mpe_freq, output_velocity_freq, attention_freq, output_onset_time, output_offset_time, output_mpe_time, output_velocity_time, midi_time.reshape([batch_size, self.n_note, self.n_frame, self.hid_dim]).permute(0, 2, 1, 3).contiguous().reshape([batch_size, self.n_frame, self.n_note* self.hid_dim])
