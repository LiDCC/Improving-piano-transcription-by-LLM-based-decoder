...
def inference_in_batch(args):
    ...
    checkpoint_path = Path("/root/autodl-tmp/Improving-piano-transcription-by-LLM-based-decoder/checkpoints/train_llama_mt_flatten_hpp_ddp/AudioLlama/step=16250_encoder.pth")
    # enc_model = HPPNet()
    ...
    # Load checkpoint
    checkpoint_path = Path("/root/autodl-tmp/Improving-piano-transcription-by-LLM-based-decoder/checkpoints/train_llama_mt_flatten_hpp_ddp/AudioLlama/step=16250.pth")
    config = EncDecConfig(
    ...
    # for audio_idx in range(3, len(audio_paths)):
        ...
        while bgn < audio_samples:
            ...
            with torch.no_grad():
                enc_model.eval()
                audio_emb = enc_model.run_on_batch(segments)
