...
def inference_in_batch(args):
    ...
    checkpoint_path = Path("checkpoints/train_llama_mt_off7/AudioLlama/step=150000_encoder.pth")
    ...
    # Load checkpoint
    checkpoint_path = Path("checkpoints/train_llama_mt_off7/AudioLlama/step=150000.pth")
    config = EncDecConfig(
        ...
        if not est_midi_path.exists():
            ...
            while bgn < audio_samples:
                ...
                with torch.no_grad():
                    ...
                    audio_emb = audio_emb.permute(0, 2, 3, 1).reshape(1, 501, 4 * 88)
