...
def train(args):
    ...
    model_name = "AudioLlama_hft_embedding_B_freeze_soft"
    ...
    # Load checkpoint
    enc_model_name = "hFT"
    # checkpoint_path = Path("checkpoints/train_llama_mt_vel_hft/AudioLlama_hft_embedding_B_freeze/step=250000_encoder.pth")
    ...
    # checkpoint_path = Path("checkpoints/train_llama_mt_vel_hft/AudioLlama_hft_embedding_B_freeze/step=250000.pth")
    ...
    for step, data in enumerate(tqdm(train_dataloader)):
        ...
        output_onset_A, output_offset_A, output_mpe_A, output_velocity_A, attention, output_onset_B, output_offset_B, output_mpe_B, output_velocity_B, midi_time, output_velocity_A_max, output_velocity_B_soft = enc_model(mel)
...
def validate(enc_model, model, dataloader):
    ...
    for step, data in tqdm(enumerate(dataloader)):
        ...
        with torch.no_grad():
            enc_model.eval()
            # vel_model.eval()
            output_onset_A, output_offset_A, output_mpe_A, output_velocity_A, attention, output_onset_B, output_offset_B, output_mpe_B, output_velocity_B, midi_time, output_velocity_A_max, output_velocity_B_soft = enc_model(
                mel)
            # vel_out_B = vel_model(output_velocity_B)
            audio_emb = torch.cat((output_onset_B, output_mpe_B, output_offset_B, output_velocity_B_soft), dim=-1)
