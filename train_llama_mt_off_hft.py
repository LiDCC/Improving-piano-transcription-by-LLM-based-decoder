...
def train(args):
    ...
    model_name = "AudioLlama_hft_embedding_B_freeze_finetune+vel_model"
    ...
    # Load checkpoint
    enc_model_name = "hFT"
    checkpoint_path = Path("checkpoints/train_llama_mt_off_hft/AudioLlama_hft_embedding_B_freeze_+vel_model/step=210000_encoder.pth")
    ...
    checkpoint_path = Path("checkpoints/train_llama_mt_off_hft/AudioLlama_hft_embedding_B_freeze_+vel_model/step=210000_vel.pth")
    ...
    checkpoint_path = Path("checkpoints/train_llama_mt_off_hft/AudioLlama_hft_embedding_B_freeze_+vel_model/step=210000.pth")
    ...
    for step, data in enumerate(tqdm(train_dataloader)):
        ...
        if step % evaluate_step_frequency == 0:
            print("Evaluating ...")
            train_loss = validate(enc_model, model, eval_train_dataloader, vel_model)
            test_loss = validate(enc_model, model, eval_test_dataloader, vel_model)
...
def validate(enc_model, model, dataloader, vel_model):
